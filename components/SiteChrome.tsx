  ...
  return (
    <header className={`site-header ${theme === 'light' ? 'site-header--light' : ''}`}>
      ...
      <nav aria-label="Navegación principal">
        ...
        <Link href="/pachakana">Pachakana</Link>
...
export function SiteFooter() {
  return (
    <footer className="site-footer">
      ...
      <small>Hecho en Guadalajara · Raíz, ritmo y barrio · 2026</small>
    </footer>
  );
}
  ...
  return (
    <section className={`event-strip ${inverse ? 'event-strip--inverse' : ''}`} aria-labelledby="eventos-titulo">
      <div className="section-heading">
        <p className="eyebrow">Convocatoria OEE</p><h2 id="eventos-titulo">Lo que se viene</h2>
        <Link href="/originalidad-estilo-esencia">Entrar a OEE <ArrowUpRight size={18} /></Link>
      </div>
